...
export type PressureCleaningTaskGuideResult = {
  ...
  overallRecommendationStatus:
    ...
    | "confirmation-required"
    | "pressure-cleaning-not-recommended"
...
function overallRecommendationLabel(
  ...
  if (status === "pressure-cleaning-not-recommended") {
    return "Pressure cleaning not recommended";
  }
...
export function travertineCanShowSoundSurfaceConfirmation(
  details: PressureCleaningJobDetails
) {
  const hasConfirmedCoreDetails =
    details.materialFinish !== "unknown" &&
    details.filledStatus !== "unknown" &&
    details.sealedStatus !== "unknown" &&
    details.jointCondition !== "unknown";
  const pressureCleaningIsBlocked =
    details.materialFinish === "polished" ||
    details.installationArea === "indoor" ||
    details.installationArea === "wall" ||
    details.installationArea === "roof" ||
    details.jointCondition === "loose-or-missing";

  return (
    hasConfirmedCoreDetails &&
    !pressureCleaningIsBlocked &&
    travertineRequiresSoundSurfaceConfirmation(details)
  );
}
...
type TaskGuidanceResolution = {
  ...
  variantLabel?: string;
  blockMessage?: string;
  blockKind?: "avoid-pressure" | "confirmation-required";
...
function resolveTaskGuidance(
  ...
  if (details.materialFinish === "polished") {
    const blockMessage =
      "Polished travertine should be cleaned using a pH-neutral stone cleaner and a non-abrasive manual or suitable machine-cleaning method. No pressure-washer nozzle setup is provided.";
    return {
      guidance: {
        mode: "avoid-pressure",
        sourceSupportsExactRange: false,
        displayWording: blockMessage,
      },
      variantLabel: "Polished travertine",
      blockMessage,
      blockKind: "avoid-pressure",
      cautionMessages: [],
    };
  }
  ...
  if (details.installationArea === "indoor") {
    const blockMessage =
      "Indoor travertine should be cleaned using a pH-neutral stone cleaner and a non-abrasive indoor stone-cleaning method. No pressure-washer nozzle setup is provided.";
    return {
      guidance: {
        mode: "avoid-pressure",
        sourceSupportsExactRange: false,
  ...
  if (details.installationArea === "wall" || details.installationArea === "roof") {
    return {
      ...
      blockKind: "confirmation-required",
      cautionMessages: [],
    };
  }
...
export function calculatePressureCleaningTaskGuide(
  ...
  const guidanceNotes = guidanceResolution.blockMessage
    ? [guidanceResolution.blockMessage]
    : [
  ...
  if (guidanceResolution.blockMessage) {
    const avoidsPressureCleaning = guidanceResolution.blockKind === "avoid-pressure";
    const overallStatus: PressureCleaningTaskGuideResult["overallRecommendationStatus"] =
      avoidsPressureCleaning
        ? "pressure-cleaning-not-recommended"
        : "confirmation-required";
    ...
    return {
      ...
      taskVariantLabel: guidanceResolution.variantLabel,
      canCalculate: false,
      validationMessages: [guidanceResolution.blockMessage],
      hydraulicCompatibility: "not-calculated",
      taskMethodCompatibility: avoidsPressureCleaning
        ? "avoid-pressure"
        : "confirmation-required",
      ...
      maxMachinePressurePsi,
      overlapStatus: "not-applicable",
      compatibilityMessages: avoidsPressureCleaning
        ? [
            "Pressure-cleaning equipment compatibility is not assessed because no pressure-washer setup is recommended for this surface variant.",
          ]
        : [],
      machineMessages: [],
      guidanceNotes,
